...
const SCOPE: &str = "identify%20guilds";
...
impl DiscordOAuth {
    ...
    pub async fn generate_authorization_url(self) -> anyhow::Result<Url> {
        ...
        Ok(Url::parse_with_params(
            ...
            &[
                ...
                ("redirect_uri", self.redirect_url.to_owned()),
                ("state", state),
    ...
    pub async fn get_user(self, code: String, state: String) -> anyhow::Result<User> {
        ...
        let code_verifier: String = redis::cmd("HGET")
            ...
            .await?;
        conn.hdel("oauth", state.to_owned()).await?;

        let form = reqwest::multipart::Form::new()
            .text("client_id", self.id.to_owned())
            .text("client_secret", self.secret.to_owned())
            .text("grant_type", GRANT_TYPE.to_string())
            .text("code", code)
            .text("redirect_uri", format!("{}/login", self.redirect_url))
            .text("code_verifier", code_verifier);
...
fn generate_state_and_code_challenge() -> (String, String, String) {
    ...
    let code_challenge =
        base64::engine::general_purpose::URL_SAFE.encode(Sha256::digest(code_verifier.as_bytes()));
